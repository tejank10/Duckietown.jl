...
function tensor_sat_test(norm, corners)
    ...
    """
    dotval = norm * corners
    numdims = ndims(dotval)
    mins = minimum(dotval, dims=numdims)
    maxs = maximum(dotval, dims=numdims)
...
function generate_corners(pos, min_coords, max_coords, theta, scale)
    """
    ...
    pz = pos[end:end]
    return [
        rotate_point(min_coords[1:1] * scale .+ px, min_coords[end:end] * scale .+ pz, px, pz, theta),
        rotate_point(max_coords[1:1] * scale .+ px, min_coords[end:end] * scale .+ pz, px, pz, theta),
        rotate_point(max_coords[1:1] * scale .+ px, max_coords[end:end] * scale .+ pz, px, pz, theta),
        rotate_point(min_coords[1:1] * scale .+ px, max_coords[end:end] * scale .+ pz, px, pz, theta),
    ]
end
...
def generate_norm(corners):
    ...
    """
    ca = np.cov(corners, y=None, rowvar=False, bias=True)
